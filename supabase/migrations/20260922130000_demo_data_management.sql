-- Admin-managed cleanup for DEMO tenants only.
-- Production tenants must cancel/reverse posted documents instead of hard deletion.

create or replace function private.rebuild_stock_balances(p_company_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  select company_id, warehouse_id, product_id, sum(qty), 0, 1
...
revoke all on function private.rebuild_stock_balances(uuid) from public, anon, authenticated;
...
revoke all on function public.admin_delete_demo_record(text,uuid,text,text) from public, anon;
revoke all on function public.admin_reset_demo_data(text,text,text) from public, anon;
grant execute on function public.admin_delete_demo_record(text,uuid,text,text) to authenticated;
grant execute on function public.admin_reset_demo_data(text,text,text) to authenticated;
