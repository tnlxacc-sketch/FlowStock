-- Only a database operator can register a verified offsite archive. The UI cannot
-- forge backup evidence; the Admin may run the rollover only after registration.
create table if not exists private.year_end_backups (
  company_id uuid not null references public.companies(id),
  closed_year integer not null check (closed_year between 2000 and 2100),
  archive_sha256 text not null check (archive_sha256 ~ '^[a-f0-9]{64}$'),
  archive_path text not null,
  backed_up_at timestamptz not null default now(),
  audit_max_id bigint not null,
  pod_objects integer not null,
  consumed_at timestamptz,
  primary key (company_id, closed_year),
  unique (archive_sha256)
);
revoke all on private.year_end_backups from public, anon, authenticated;
...
as $$
declare ctx record; backup record; company_code text; carry_id uuid; carry_ref text;
  carried integer; archived_orders bigint; archived_invoices bigint;
begin
  select * into ctx from private.require_role(array['ADMIN']);
  if p_year not between 2000 and extract(year from now())::integer-1
    then raise exception 'YEAR_NOT_FINISHED'; end if;
  ...
  -- Preserve POD objects in the verified archive; remove their metadata only.
...
revoke all on function public.admin_rollover_year(integer,text,text,text) from public,anon;
grant execute on function public.admin_rollover_year(integer,text,text,text) to authenticated;
