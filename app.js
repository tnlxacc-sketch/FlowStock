...
const roleMenus={
  ...
  WAREHOUSE:[['dashboard','⌂','หน้าหลัก'],['warehouse','▣','งานคลัง'],['stock','▦','Stock'],['counts','✓','ตรวจนับ Stock'],['transfers','⇄','โอนคลัง'],['repack','↔','Repack / Conversion']],
...
const PAGE_HELP={
  ...
  counts:{title:'ตรวจนับ Stock',steps:['เริ่มรอบตรวจนับที่คลัง','กรอกยอดตรวจจริงให้ครบ','ส่งให้ Admin Final','เมื่อ Final ระบบจึงปรับ Stock ตามส่วนต่าง']},
...
function settingsPage(){
  const policy=state.data.settings?.find(x=>x.setting_key==='contribution_alert_pct'),threshold=policy==null?12:Number(policy.setting_value),
    stockPolicy=orderStockPolicy(),numberPolicy=orderNumberPolicy(),current=new Date().toISOString().slice(0,7),
